-- ==============================================================================
-- 1. POBLAR CATÁLOGOS Y DIMENSIONES (50 registros cada uno)
-- ==============================================================================

INSERT INTO dim_ramo (cod_ramo, ramo)
SELECT i, 'Ramo ' || i FROM generate_series(1, 50) AS i;

INSERT INTO dim_agrupador (cod_agrupador_principal, agrupador_principal, cod_agrupador_secundario, agrupador_secundario)
SELECT i, 'Agrupador P. ' || i, i, 'Agrupador S. ' || i FROM generate_series(1, 50) AS i;
...
SELECT i, 'Agente Principal ' || i, i, 'Agente Sec ' || i, 1000 + i, '17200' || lpad(i::text, 5, '0') FROM generate_series(1, 50) AS i;

INSERT INTO dim_punto_venta (cod_punto_venta, punto_venta)
SELECT i, 'Punto Venta ' || i FROM generate_series(1, 50) AS i;

INSERT INTO dim_ejecutivo (cod_ejecutivo, ejecutivo)
SELECT i, 'Ejecutivo ' || i FROM generate_series(1, 50) AS i;

INSERT INTO dim_canal_emision (cod_canal, canal_emision)
SELECT i, 'Canal Emisión ' || i FROM generate_series(1, 50) AS i;

INSERT INTO tpais (cod_pais, txt_desc)
SELECT i, 'País ' || i FROM generate_series(1, 50) AS i;

INSERT INTO tdpto (cod_dpto, cod_pais, txt_desc)
SELECT i, i, 'Departamento ' || i FROM generate_series(1, 50) AS i;

INSERT INTO tmunicipio (cod_municipio, cod_pais, cod_dpto, txt_desc)
SELECT i, i, i, 'Municipio ' || i FROM generate_series(1, 50) AS i;

INSERT INTO tcausa_stro (cod_causa, cod_ramo, txt_desc)
SELECT i, i, 'Causa Siniestro ' || i FROM generate_series(1, 50) AS i;


-- ==============================================================================
-- 2. POBLAR TABLAS MAESTRAS DE PÓLIZAS Y VEHÍCULOS (50 registros)
-- ==============================================================================
...
SELECT i, i, i, 10000 + i, 2026, i, i, '2026-01-01'::date + i, '2026-01-01'::date + i, '2026-12-31'::date, i, i, i, i, i FROM generate_series(1, 50) AS i;
...
SELECT i, '2026-01-01'::date, '2026-12-31'::date, i, 'Sucursal ' || i, i, i, i, 'Nombre ' || i, 'Apellido ' || i, '09900' || lpad(i::text, 5, '0'), i, 'Producto Vehículos ' || i FROM generate_series(1, 50) AS i;
...
SELECT i, i, i, 'Marca ' || i, i, 'Modelo ' || i, 'CHAS' || i || 'X99', 'PCH-' || lpad(i::text, 4, '0'), i, 'Color ' || i, i, 'Tipo ' || i, i, 'Clase ' || i, i, 'Uso ' || i, 2015 + (i % 10), 'Dirección Hogar/Pyme ' || i FROM generate_series(1, 50) AS i;

INSERT INTO tb_cobertura (cod_cober, id_poliza, cod_item, cobertura, sn_activo)
SELECT i, i, i, 'asistencia vehicular', true FROM generate_series(1, 50) AS i;


-- ==============================================================================
-- 3. POBLAR MÓDULO DE SINIESTROS Y RESERVAS (50 registros)
-- ==============================================================================
...
SELECT i, i, i, 2026, 'STRO-2026-' || i, i, '2026-02-01'::date + (i * 2), '2026-02-02'::date + (i * 2), '2026-02-03'::date + (i * 2), i, 'UsuSist_' || i FROM generate_series(1, 50) AS i;
...
SELECT i, i, i, 5000 + i, '2026-03-01'::date + i, '2026-03-10'::date + i, '2026-03-05'::date + i, '2026-03-08'::date + i, null, (random() * 1500)::numeric(15,2), 'Pago Aut ' || i, i, i, i, i, i, 'Beneficiario ' || i, i, 'Taller Razon Social ' || i, 'UsuCaja_' || i, 'Reclamante ' || i FROM generate_series(1, 50) AS i;
...
SELECT i, i, i, i, i, i, (random() * 800)::numeric(15,2), i, i FROM generate_series(1, 50) AS i;

INSERT INTO estim_header (id_stro, cod_item, cod_ind_cob, txt_bien_strado, cod_municipio, cod_dpto, cod_pais, id_pv, cod_ramo)
SELECT i, i, i, 'Bien Asegurado ' || i, i, i, i, i, i FROM generate_series(1, 50) AS i;
...
SELECT i, i, i, i, i, '2026-02-15'::date + i, i, (random() * 2000)::numeric(15,2), 'UsuReserva_' || i FROM generate_series(1, 50) AS i;


-- ==============================================================================
-- 4. POBLAR TABLAS COMERCIALES (50 registros)
-- ==============================================================================
...
SELECT i, 'Nueva', 2026, (i % 12) + 1, 'Mes ' || ((i % 12) + 1), '2026-01-01'::date + i, 'Sucursal ' || i, i, i, 'Broker', i, 'Agente ' || i, i, i, i, 'Contratante ' || i, i, 'Asegurado ' || i, '1700' || lpad(i::text, 6, '0'), '2026-01-01'::date + i, '2026-01-01'::date + i, '2026-12-31'::date, 'Directo', 'Grupo ' || i, 'Holding ' || i, (random() * 5000)::numeric(15,2), 'PV ' || i, 'Emisión', 'Alta Nueva', 'SI', 'Tarjeta', 'Ejecutivo ' || i FROM generate_series(1, 50) AS i;
...
SELECT i, 'Presupuestado', 2026, (i % 12) + 1, 'Mes ' || ((i % 12) + 1), '2026-01-01'::date + i, i, i, 'Broker', i, 'Agente ' || i, 'Directo', 'Grupo ' || i, 'Holding ' || i, (random() * 10000)::numeric(15,2), 'PV ' || i, 'SI', 'Ejecutivo ' || i FROM generate_series(1, 50) AS i;
...
SELECT 2026, (i % 12) + 1, 'Mes ' || ((i % 12) + 1), '2026-01-01'::date + i, 'Sucursal ' || i, i, i, 'Broker', i, 'Agente ' || i, i, i, i, 'Contratante ' || i, i, 'Asegurado ' || i, '1700' || lpad(i::text, 6, '0'), '2026-01-01'::date, '2026-12-31'::date, 'Directo', 'Grupo ' || i, 'Holding ' || i, (random() * 3000)::numeric(15,2), 'PV ' || i, 'Ejecutivo ' || i, 'Pendiente' FROM generate_series(1, 50) AS i;
...
SELECT 2026, (i % 12) + 1, 'Mes ' || ((i % 12) + 1), '2026-01-01'::date + i, i, i, 'Broker', i, 'Agente ' || i, 'Brokerage', (random() * 5000)::numeric(15,2), 'PV ' || i, 'Ejecutivo ' || i, 'Prospecto ' || i, 'Q-' || (100+i), (random() * 100)::int || '%', 'Negociación', 'Abierto' FROM generate_series(1, 50) AS i;

INSERT INTO asesores_productores_seguros_scvs (agente, codigo_super_cias, ramo, prima, fecha)
SELECT 'Agente SCVS ' || i, 9000 + i, 'Ramo ' || i, (random() * 8000)::numeric(15,2), '2026-01-01'::date + i FROM generate_series(1, 50) AS i;


-- ==============================================================================
-- 5. POBLAR BANCA SEGUROS / PAGOS MASIVOS (50 registros)
-- ==============================================================================
...
SELECT 'CLI-' || i, 'Asegurado EM ' || i, '2026-01-01'::date + i, 'Desgravamen', 'Vida Segura', 'Sucursal ' || (i%5), 'P_Ape ' || i, 'S_Ape ' || i, 'Nombres ' || i, (random() * 200)::numeric(15,2) FROM generate_series(1, 50) AS i;
...
SELECT 'RFC' || lpad(i::text, 4, '0'), 'Segmento VIP', '4000-1234-5678-' || lpad(i::text, 4, '0'), 'Afiliado Tarjeta ' || i, '2026-01-01'::date + i, 'OF-2026-' || i, 'Carga ' || i, 'Mes ' || ((i % 12) + 1), (random() * 50)::numeric(15,2), 'Cobrado', 'Asistencia Full', 'Plan Tranquilidad' FROM generate_series(1, 50) AS i;

INSERT INTO sabana_pagos_emasivos (campo_ultimo_mes, campo_cedula, campo_forma_pago)
SELECT 'Mes ' || ((i % 12) + 1), '099' || lpad(i::text, 7, '0'), 'Débito Cuenta' FROM generate_series(1, 50) AS i;